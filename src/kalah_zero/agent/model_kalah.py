...
class KalahModel:
    ...
    def build(self):
        ...
        in_x = x = Input((2, 14))  # [own(14), enemy(14)]

        # (batch, channels, height, width)
        x = Conv1D(filters=mc.cnn_filter_num, kernel_size=mc.cnn_filter_size, padding="same",
        ...
        res_out = x
        # for policy output
        x = Conv1D(filters=2, kernel_size=1, kernel_regularizer=l2(mc.l2_reg))(res_out)
        x = BatchNormalization(axis=1)(x)
        x = Activation("relu")(x)
        x = Flatten()(x)
        # no output for 'pass'
        ...
        # for value output
        x = Conv1D(filters=1, kernel_size=1, kernel_regularizer=l2(mc.l2_reg))(res_out)
        x = BatchNormalization(axis=1)(x)
        x = Activation("relu")(x)
        x = Flatten()(x)
        ...
        self.model = Model(in_x, [policy_out, value_out], name="kalah_model")
